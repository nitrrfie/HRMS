...
const EFiling = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (data.success) {
        ...
        const others = filteredByLevel.filter(
          (emp) => emp._id !== user?.id && emp.username !== user?.username
        );
        setEmployees(others);
      }
    } catch (error) {
  ...
  const getFileIcon = (fileType) => {
    ...
    if (fileType?.includes("presentation") || fileType?.includes("powerpoint"))
      return <Presentation size={20} className="file-icon ppt" />;
    if (fileType?.includes("zip") || fileType?.includes("rar"))
      return <Archive size={20} className="file-icon archive" />;
    ...
    <div className="send-section">
      <div className="send-form">
                    ...
                    filteredEmployees.map((emp) => (
                      <div
                        ...
                        className={`dropdown-item ${
                          selectedRecipient === emp._id ? "selected" : ""
                        }`}
                        onClick={() => {
            ...
            {selectedFile ? (
              ...
              <div className="upload-placeholder">
                ...
                <span className="file-types">
                  PDF, DOC, DOCX, XLS, XLSX, PPT, PPTX, Images, ZIP (Max 25MB)
                </span>
              </div>
            )}
          </div>
        ...
        <button
          className="send-btn"
          onClick={handleSend}
          disabled={loading || !selectedFile || !selectedRecipient}
        >
          {loading ? (
            <span>Sending...</span>
          ) : (
      ...
      {loading ? (
        ...
        <div className="file-list">
          {inbox.map((transfer) => (
            <div
              key={transfer._id}
              className={`file-item ${!transfer.isRead ? "unread" : ""}`}
            >
              ...
              <div className="file-details">
                ...
                <div className="file-meta">
                  <span className="sender">
                    <User size={14} /> From:
                    {/* {getEmployeeName(transfer.sender)} */}
                    {transfer.sender?.employment?.designation && (
                      <span className="sender-designation">
                        {transfer.sender.employment.designation}
                      </span>
                    )}
                  </span>
                  <span className="date">
                    <Clock size={14} /> {formatDateTime(transfer.createdAt)}
                  </span>
                </div>
                {transfer.note && (
                  <div className="file-note">
                    <Paperclip size={14} /> {transfer.note}
                  </div>
                )}
              </div>
              <div className="file-actions">
                ...
                {!transfer.isRead && (
                  <button
                    ...
                    onClick={() => handleMarkAsRead(transfer._id)}
                    title="Mark as read"
                  >
                    <Check size={18} />
                  </button>
                  ...
                  {forwardDropdownOpen === transfer._id && (
                    <div className="forward-dropdown">
                      <div className="dropdown-header">
                        <h4>Forward to:</h4>
                        <button
                          onClick={() => setForwardDropdownOpen(null)}
                          className="close-btn"
                        >
                          <X size={16} />
                        </button>
                        ...
                        {employees
                          ...
                          .map((emp) => (
                            <div
                              ...
                              className="dropdown-item"
                              onClick={() =>
                                handleForwardFile(transfer._id, emp._id)
                              }
                            >
                              <User size={16} />
  ...
  const handleForward = async () => {
    ...
    try {
      ...
      if (selectedThread) {
        const threadData = await efilingAPI.getFileThread(
          selectedThread[0]._id
        );
        ...
        <div className="journey-header">
          ...
          <div className="file-info">
            <div className="file-icon-large">
              {getFileIcon(selectedThread[0]?.fileType)}
            </div>
            <div>
              <h3>{selectedThread[0]?.originalName}</h3>
              <p>
                {formatFileSize(selectedThread[0]?.fileSize)} •{" "}
                {formatDateTime(selectedThread[0]?.createdAt)}
              </p>
            </div>
          </div>
        </div>
              ...
              return (
                ...
                >
                  {displayRow.map((node, displayIndex) => {
                    ...
                    const dataIndex = isReversed
                      ? row.length - 1 - displayIndex
                      : displayIndex;
                    const originalIndex = rowIndex * itemsPerRow + dataIndex;

                    // Determine if this step needs connectors
                    const isLastInDisplay =
                      displayIndex === displayRow.length - 1;
                    ...
                    const userName = node.user?.profile?.firstName
                      ? `${node.user.employment.designation}`
                    ...
                    return (
                      <div
                        ...
                        className={`timeline-step ${
                          node.isCurrentHolder ? "current-holder" : ""
                        } ${hasHorizontalConnector ? "has-horizontal" : ""} ${
                          hasVerticalConnector ? "has-vertical" : ""
                        }`}
          ...
          >
            ...
            >
              ...
              <div className="modal-header-section">
                <div className="user-avatar-large">
                  {selectedNode.user?.profile?.firstName
                    ? `${selectedNode.user.profile.firstName[0]}${
                        selectedNode.user.profile.lastName?.[0] || ""
                      }`
                ...
                <div>
                  <h3>
                    {selectedNode.user?.profile?.firstName
                      ? `${selectedNode.user.profile.firstName} ${
                          selectedNode.user.profile.lastName || ""
                        }`
                      : selectedNode.user?.username || "Unknown"}
                  </h3>
                  <p>
                    {selectedNode.user?.employment?.designation || "Employee"}
                  </p>
                </div>
              </div>
              ...
              <div className="modal-detail-row">
                ...
                <div
                  className={`status-badge-large status-${selectedNode.status}`}
                >
                  {selectedNode.status === "sent" && (
                    <>
                      <Send size={14} /> Sent
                    </>
                  )}
                  {selectedNode.status === "delivered" && (
                    <>
                      <CheckCircle2 size={14} /> Delivered
                    </>
                  )}
                  {selectedNode.status === "read" && (
                    <>
                      <Eye size={14} /> Read/Approved
                    </>
                  )}
                </div>
              </div>
                    ...
                    <div className="file-meta">
                      <span>
                        ...
                        {transfer.sender?.employment?.designation ||
                          ...
                          getEmployeeName(transfer)}
                      </span>
                      <span>
                        To:{" "}
                        {transfer.recipient?.employment?.designation ||
                          transfer.recipient?.role ||
                          getEmployeeName(transfer.recipient)}
                      </span>
                      <span className="date">
                        <Clock size={14} /> {formatDateTime(transfer.createdAt)}
                      </span>
            ...
            <div className="form-group">
              ...
              <div className="recipient-selector">
                ...
                >
                  <User size={18} />
                  <span
                    className={selectedRecipient ? "selected" : "placeholder"}
                  >
                    {selectedRecipient
                      ? getSelectedRecipientName()
                      : "Choose recipient..."}
        ...
        <div className="file-list history-list">
          {history.map((transfer) => {
            const isSent =
              ...
              transfer.sender?.username === user?.username;
            return (
              <div
                key={transfer._id}
                className={`file-item ${isSent ? "sent" : "received"}`}
              >
